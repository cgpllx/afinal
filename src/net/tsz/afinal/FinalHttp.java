...
public class FinalHttp {
	...
	public FinalHttp() {
		...
		ConnManagerParams.setMaxConnectionsPerRoute(httpParams, new ConnPerRouteBean(maxConnections)); // 设置每个路由最大连接数
		ConnManagerParams.setMaxTotalConnections(httpParams, 10);// 设置最大连接数
		HttpConnectionParams.setSoTimeout(httpParams, socketTimeout); // 设置获取连接的最大等待时间
		HttpConnectionParams.setConnectionTimeout(httpParams, socketTimeout); // 设置连接超时时间
		HttpConnectionParams.setTcpNoDelay(httpParams, true);// 设置是否延迟发送
		HttpConnectionParams.setSocketBufferSize(httpParams, DEFAULT_SOCKET_BUFFER_SIZE);

		HttpProtocolParams.setVersion(httpParams, HttpVersion.HTTP_1_1);
		// Scheme类代表了一个协议模式，比如“http”或“https”同时包含一些协议属性，比如默认端口，//
		// 用来为给定协议创建java.net.Socket实例的套接字工厂。SchemeRegistry类用来维持一组Scheme，//
		// 当去通过请求URI建立连接时，HttpClient可以从中选择：
		SchemeRegistry schemeRegistry = new SchemeRegistry();
		schemeRegistry.register(new Scheme("http", PlainSocketFactory.getSocketFactory(), 80));
		schemeRegistry.register(new Scheme("https", SSLSocketFactory.getSocketFactory(), 443));
		ThreadSafeClientConnManager cm = new ThreadSafeClientConnManager(httpParams, schemeRegistry);// ThreadSafeClientConnManager用来支持多线程请求
		// ClientConnectionManager d;
		httpContext = new SyncBasicHttpContext(new BasicHttpContext());// 同步
		httpClient = new DefaultHttpClient(cm, httpParams);
		httpClient.addRequestInterceptor(new HttpRequestInterceptor() {// 请求拦截器
					public void process(HttpRequest request, HttpContext context) {
						if (!request.containsHeader(HEADER_ACCEPT_ENCODING)) {
							request.addHeader(HEADER_ACCEPT_ENCODING, ENCODING_GZIP);// 设置相应相应的拦截器,用于处理接收到的拦截的压缩信息
						}
		...
		httpClient.addResponseInterceptor(new HttpResponseInterceptor() {// 响应拦截器
		...
		httpClient.setHttpRequestRetryHandler(new RetryHandler(maxRetries));//设置重试次数
